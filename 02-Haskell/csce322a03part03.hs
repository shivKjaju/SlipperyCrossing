...
manyPlayersOneSlide ma mos = playerMaker ma mos

controller2 :: [[Char]] -> [Char] -> [Char] -> [[Char]]
controller2  maze _ [] = maze
controller2 maze (h:moveTail) (player:theTail) = controller2 (movePlayer maze h player) moveTail theTail
    
playerMaker :: [[Char]] -> [Char] -> [[Char]]
playerMaker maze move = controller2 maze move (sort (convertTo1D (loopThrough maze)))

-- loop through the given maze; find the players make a [char] of players

loopThrough :: [[Char]] -> [[Char]]
loopThrough [] = []
loopThrough (row:rows)  = filter (not . null) ((filter (\x -> x/='x' && x/='-' && x/='g') row) : loopThrough rows)
